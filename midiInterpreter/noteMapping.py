...
def getMusic(chosenSong):
	...
	returnedStuff = midi.parse(chosenSong)
	...
	for c in chords: #example: [(0, 4), (9, 3), 120]
		...
		for t in reversed(range(len(c)-1)): #for each tuple item, excluding the time, backwards.
											#this is because parseMidi formats the notes highest first, and
			...
			relativeDist = {}
			for k, v in rawPosStrings.items(): #for string(k), fret(v) in all possible string positions
	...
	return tempoChanges, railPos, pickStrings
